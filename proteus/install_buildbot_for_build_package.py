...
version = 'develop'
proteus_deploy_git_url = 'git+git://github.com/Proteus-tech/deploy.git@%s' % \
                         (version)
...
def create_virtenv_base_project(server, project_name, virtenv_name):
    ...
    with cd(project_base_folder):
        ...
        with prefix("source %s/bin/activate" % (virtenv_name)):
            sudo("pip install buildbot-slave==0.8.5", user="www-data")
            sudo(
                "pip install {0}".format(proteus_deploy_git_url),
                user="www-data"
            )
